...
export const getCurrentMonthOrders = async () => {
  ...
  const startOfMonth = new Date(Date.UTC(currentDate.getFullYear(), currentDate.getMonth(), 1, 0, 0, 0))
  // first day of the current month at 00:00:00
  const startOfNextMonth = new Date(Date.UTC(currentDate.getFullYear(), currentDate.getMonth() + 1, 1, 0, 0, 0))
  // first day of the next month at 00:00:00

  const orders = await prisma.order.findMany({
    where: {
      createdAt: {
        gte: startOfMonth, // from the start of the current month
        lt: startOfNextMonth // to the start of the next month
      }
    }
  })
