...
import { z } from 'zod'
import { StockDetails } from './SellerProductFormStockDetails'
...
export const ProductSellerForm = ({ product, categories }: Props) => {
  ...
  async function onSubmit(values: z.infer<typeof productSchema>) {
    ...
    if (totalStock <= 0) {
      noticeFailure('Debes ingresar al menos un stock')
      setIsSubmitting(false)
      return
    }

    if (images.length < 2) {
      noticeFailure('Debes ingresar al menos dos imágenes')
      setIsSubmitting(false)
      return
    }
    ...
    if (!ok) {
      noticeFailure('No se pudo guardar el producto, intente nuevamente')
      setIsSubmitting(false)
      return
    }

    setIsSubmitting(false)
    noticeSuccess('Producto guardado correctamente')
